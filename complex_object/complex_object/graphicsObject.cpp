...
void GraphicsObject::setParent(GraphicsObject * myParent)
{
	parent = myParent;
}
...
Matrix4f const GraphicsObject::getTransformationMatrix()
{
	Matrix4f rotMat;  // rotation matrix;
	Matrix4f scaleMat; // scaling matrix;
	Matrix4f transMat;	// translation matrix
	...
	//applying parent transformation
	if (parent != NULL){
		modelMat = parent->getTransformationMatrix() * modelMat;
	}
